use std::net::IpAddr;

fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Testing certificate conversion locally...");
    
    // Test PEM parsing with a sample certificate
    let sample_cert = "-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----";

    // Test base64 decoding
    let mut cert_data = String::new();
    let mut in_cert = false;
    
    for line in sample_cert.lines() {
        if line == "-----BEGIN CERTIFICATE-----" {
            in_cert = true;
            cert_data = String::new();
        } else if line == "-----END CERTIFICATE-----" {
            in_cert = false;
            if !cert_data.is_empty() {
                println!("✅ Found certificate data, length: {}", cert_data.len());
                
                // Test base64 decoding
                match base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &cert_data) {
                    Ok(der) => {
                        println!("✅ Successfully decoded certificate DER, length: {}", der.len());
                        println!("Certificate conversion test PASSED!");
                        return Ok(());
                    }
                    Err(e) => {
                        println!("❌ Failed to decode certificate: {}", e);
                        return Err(e.into());
                    }
                }
            }
        } else if in_cert {
            cert_data.push_str(line);
        }
    }
    
    println!("❌ No certificate found in sample data");
    Err("No certificate found".into())
}

