//! Test OpenSSL 3.5 from the Nix store.
//!
...
fn test_nix_openssl_35() {
    ...
    let (libssl, libcrypto) = match common::find_openssl_glob("*-openssl-3.5.*") {
        Some(p) => p,
        None => {
            eprintln!("OpenSSL 3.5 not found in Nix store, skipping");
            return;
        }
    };

    eprintln!("OpenSSL 3.5:");
    ...
    let ssl = match lsb_openssl::Openssl::load() {
        ...
        Err(e) => panic!("OpenSSL 3.5 load failed: {e}"),
    };
    ...
    common::test_openssl_load_cert(&ssl, "OpenSSL 3.5");
    common::test_openssl_handshake(&ssl, "OpenSSL 3.5");
