...
//!
//! # Key Design Feature: On-Demand ACME Certificate Acquisition
//!
//! EasyP's core differentiator is that it triggers ACME certificate requests
//! **on-demand** from the TLS SNI callback — no domains need to be pre-configured.
//! When an HTTPS request arrives for a domain not yet in the cert cache:
//!
//! 1. The SNI callback serves a self-signed fallback certificate immediately
//!    (so the connection isn't dropped).
//! 2. An async ACME task is spawned in the background to request a real
//!    Let's Encrypt certificate for that domain.
//! 3. The ACME account is created automatically using `webmaster@DOMAIN`
//!    as the email (if no `--email` was provided).
//! 4. Once the certificate is obtained, it is cached in memory (ptr_cache)
//!    and persisted to `{cache_dir}/{domain}/fullchain.pem` + `privkey.pem`.
//! 5. Subsequent TLS connections for that domain serve the real ACME cert.
//!
//! This means EasyP works with ZERO configuration:
//!   easyp --root /var/www/html
//!
//! Just point your domain's DNS at the server and it Just Works™.
//!
//! Usage:
...
use crate::acme_lsb::AcmeLsbClient;
...
#[cfg(feature = "crypto-lsb")]
#[path = "../modules/acme_lsb.rs"]
mod acme_lsb;

#[cfg(feature = "crypto-rustls")]
type AcmeClientType = Arc<rustls_acme::AcmeClient>;
#[cfg(feature = "crypto-lsb")]
type AcmeClientType = Arc<crate::acme_lsb::AcmeLsbClient>;
        ...
        while let Some(arg) = parser.next()? {
            match arg {
                ...
                Long("help") => {
                    ...
                    println!("        --http2                            Enable HTTP/2 via ALPN negotiation (requires libnghttp2) [default: on]");
                    println!("        --no-http2                         Disable HTTP/2");
                    println!("        --http3                            Enable HTTP/3 over QUIC (WIP — basic skeleton, requires libngtcp2 + libnghttp3)");
                    println!("        --no-http3                         Disable HTTP/3");
...
struct SniFallback {
    ...
    /// On-demand ACME trigger — called from SNI callback when an unknown domain is requested.
    /// The closure receives the SNI hostname and should spawn async ACME cert acquisition.
    #[cfg(feature = "acme")]
    acme_trigger: std::sync::Mutex<Option<Box<dyn Fn(String) + Send + Sync + 'static>>>,
    /// Set of domains for which an on-demand ACME request is in flight.
    /// Used to avoid duplicate concurrent requests for the same domain.
    #[cfg(feature = "acme")]
    pending_domains: std::sync::Mutex<std::collections::HashSet<String>>,
}
        ...
        if let Ok(x509) = lsb_openssl::certs::pem_read_certificate(cert_pem) {
            if let Ok(pkey) = lsb_openssl::certs::pem_read_private_key(key_pem) {
                if let Ok(mut cache) = self.ptr_cache.write() {
                    println!("✅ Cached ACME cert for {} in SNI callback (ptr_cache)", hostname);
                    cache.insert(hostname.to_string(), (x509, pkey));
                }
            } else {
                eprintln!("⚠️  Failed to parse private key for {} in cache_cert", hostname);
            }
        } else {
            eprintln!("⚠️  Failed to parse certificate for {} in cache_cert", hostname);
        }
    }

    /// Set the on-demand ACME trigger closure.
    #[cfg(feature = "acme")]
    fn set_acme_trigger<F>(&self, f: F)
    where
        F: Fn(String) + Send + Sync + 'static,
    {
        if let Ok(mut trigger) = self.acme_trigger.lock() {
            *trigger = Some(Box::new(f));
        }
    }

    /// Remove a domain from the pending set (call after ACME completes or fails).
    #[cfg(feature = "acme")]
    fn clear_pending(&self, domain: &str) {
        if let Ok(mut pending) = self.pending_domains.lock() {
...
/// Trigger on-demand ACME for a domain not yet in the cert cache.
/// This is called from the SNI callback when no cached cert is found.
/// It fires the trigger once per domain (domains in flight are tracked).
#[cfg(feature = "acme")]
fn trigger_on_demand_acme(state: &SniFallback, domain: &str) {
    // Deduplicate: skip if a request is already in flight for this domain
    let is_pending = {
        if let Ok(pending) = state.pending_domains.lock() {
            if pending.contains(domain) {
                true
            } else {
                state.clear_pending(domain); // ensure clean
                false
            }
        } else {
            true
    ...
    if is_pending {
        println!("⚠️  ACME request already in flight for {}, skipping duplicate trigger", domain);
        return;
    }

    println!("🔍 On-demand ACME triggered for domain: {}", domain);

    // Mark as pending
    if let Ok(mut pending) = state.pending_domains.lock() {
        pending.insert(domain.to_string());
    }

    // Fire the trigger closure
    if let Ok(trigger_guard) = state.acme_trigger.lock() {
        if let Some(ref trigger) = *trigger_guard {
            trigger(domain.to_string());
        } else {
            eprintln!("⚠️  ACME trigger not set for domain: {}", domain);
            state.clear_pending(domain);
        }
    } else {
        state.clear_pending(domain);
    }
...
unsafe extern "C" fn sni_callback(
    ...
    if let Some(ref hostname) = hostname {
        ...
        // Not in cache: trigger on-demand ACME in the background,
        // then fall through to serve the self-signed cert for THIS connection.
        #[cfg(feature = "acme")]
        trigger_on_demand_acme(state, hostname);
    }
...
struct OnDemandHttpsServer {
    ...
    #[cfg(feature = "acme")]
    acme_client: Option<AcmeClientType>, // ACME client for challenge responses
...
*/
                ...
                // Use the pre-drop check result from earlier.
                 ...
                 let (cert_path, key_path) = ensure_self_signed_cert("localhost", &args.cache_dir)
                  ...
                  let (sni_x509, sni_pkey) = load_self_signed_ptrs(&cert_path, &key_path)?;
                   ...
                   // Also scan cache dir for any on-demand persisted certs (directories with fullchain.pem)
                   if let Ok(entries) = std::fs::read_dir(&args.cache_dir) {
                       for entry in entries.flatten() {
                           let path = entry.path();
                           if path.is_dir() {
                               if let Some(dir_name) = path.file_name().and_then(|n| n.to_str()) {
                                   if dir_name == "localhost" || dir_name == "acme_lib" || args.domains.contains(&dir_name.to_string()) {
                                       continue; // Already handled or not a domain
                                   }
                                   ...
                                   if cert_file.exists() && key_file.exists() {
                                       if let Ok(cert_pem) = std::fs::read_to_string(&cert_file) {
                                           if let Ok(key_pem) = std::fs::read_to_string(&key_file) {
                                               sni_fallback.cache_cert(dir_name, &cert_pem, &key_pem);
                                               println!("✅ Loaded on-demand cached cert for {} from disk", dir_name);
                                           }
                                       }
                                   }
                               }
                 ...
                 #[cfg(all(feature = "acme", feature = "crypto-rustls"))]
                   let acme_client: Option<AcmeClientType> = {
                       ...
                       let email = args.email.clone()
                           ...
                           .unwrap_or_default();
                       let config = rustls_acme::AcmeConfig {
                           directory_url,
                           email,
                           allowed_ips: allowed_ips.clone(),
                           challenge_type: rustls_acme::ChallengeType::Http01(String::new(), String::new()),
                       ...
                       if !args.domains.is_empty() {
                           match client.initialize_account().await {
                               ...
                               Err(e) => {
                                   ...
                                   println!("ℹ️  Continuing with self-signed certificates — on-demand ACME still active");
                                   Some(client)
                               }
                           }
                       } else {
                           println!("ℹ️  No domains configured — ACME will be triggered on-demand via SNI callback");
                           Some(client)
                       }
                   };
                   ...
                   let acme_client: Option<AcmeClientType> = {
                       ...
                       let email = args.email.clone()
                           .or_else(|| args.acme_email.clone())
                           .unwrap_or_default();
                       let http_challenges = Arc::new(std::sync::Mutex::new(std::collections::BTreeMap::new()));
                       let client = Arc::new(crate::acme_lsb::AcmeLsbClient::new(
                           directory_url,
                           email,
                   ...
                   if let Some(ref ac_client) = acme_client {
                      ...
                      sni_fallback.set_acme_trigger(move |domain: String| {
                          println!("📡 On-demand ACME triggered for: {}", domain);
                          let sni = sni.clone();
                          let ac = ac.clone();
                          let cache_dir = cache_dir.clone();
                          // Spawn a dedicated OS thread so synchronous I/O (minreq, thread::sleep)
                          // doesn't stall the async executor.
                          let domain2 = domain.clone();
                          let sni2 = sni.clone();
                          std::thread::spawn(move || {
                              let result = smol::block_on(async move {
                                  println!("🔍 On-demand ACME: requesting certificate for: {}", domain);
                                  match ac.get_certificate_pem(&domain).await {
                                      Ok((cert_pem, key_pem)) => {
                                          println!("✅ On-demand ACME certificate obtained for: {}", domain);
                                          sni.cache_cert(&domain, &cert_pem, &key_pem);
                                          let domain_dir = format!("{}/{}", cache_dir, domain);
                                          let _ = std::fs::create_dir_all(&domain_dir);
                                          let cert_path = format!("{}/fullchain.pem", domain_dir);
                                          let key_path = format!("{}/privkey.pem", domain_dir);
                                          let _ = std::fs::write(&cert_path, &cert_pem);
                                          let _ = std::fs::write(&key_path, &key_pem);
                                          true
                                      }
                                      Err(e) => {
                                          eprintln!("⚠️  On-demand ACME failed for {}: {}", domain, e);
                                          false
                                      }
                                  }
                              });
                              if !result {
                                  eprintln!("⚠️  On-demand ACME failed for {}", domain2);
                              }
    ...
    async fn handle_http_connection(
        ...
        #[cfg(feature = "acme")]
        acme_client: Option<AcmeClientType>,
    ...
    async fn handle_https_connection_async(
        ...
        if args.http2_enabled {
            if alpn.as_deref() == Some(b"h2") {
                ...
                h2_handler.set_extension_registry(extension_registry.clone());
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if log::set_logger(&SimpleLogger).is_err() {
        eprintln!("Warning: Logger already set, continuing with existing logger");
    }
