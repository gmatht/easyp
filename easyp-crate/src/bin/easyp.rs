...
*/
    ...
    async fn handle_https_connection_async(
        ...
        let mut server_config = TokioServerConfig::builder_with_provider(
            rustls::crypto::ring::default_provider().into()
        )
        .with_no_client_auth()
        .with_cert_resolver(cert_resolver)?;

        // Advertise h2 via ALPN when HTTP/2 is enabled
        #[cfg(feature = "http2")]
        ...
        if args.http2_enabled {
            ...
            if alpn == Some(b"h2") {
                ...
                h2_handler.upgrade_connection(&mut tls_stream, "").await?;
                return Ok(());
            }
        }
        ...
        loop {
            ...
            if !should_keep_alive {
                println!("Closing async HTTPS connection after {} requests (version: {})",
                    request_count, http_version);
            ...
            println!("Keeping async HTTPS connection alive for next request (version: {})",
                http_version);
        }
